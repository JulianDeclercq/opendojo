...
// D3D12 render hook. Installs vtable hooks on IDXGISwapChain::Present and
// ID3D12CommandQueue::ExecuteCommandLists, plus a WndProc hook for input.
// All real work happens off the loader lock on a worker thread.
//
...
// dxgi.dll to be loaded, creates a dummy device/queue/swapchain to grab the
// vtable function pointers, then atomically swaps them with our hooks.
