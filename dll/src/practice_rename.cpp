...
void* find_class_by_name(FindObjectsOfClassFn fo,
...
void do_resolve() {
    ...
    if (!h1 || !h2) {
        OPENDOJO_LOG("practice_rename: pattern scan miss (fuc=%d foc=%d)",
                     h1 ? 1 : 0, h2 ? 1 : 0);
        return;
    }
    ...
    g_r.cls_gryphon    = g_r.find_class(nullptr, L"/Script/GryphonLocalization.GryphonFunctionLibrary", true);
    ...
    OPENDOJO_LOG("practice_rename: engine resolve %s — find_class=0x%llX "
...
void try_resolve_bp_classes() {
    ...
    if (!g_r.cls_button_row) {
        OPENDOJO_LOG("practice_rename: WBP_UI_PracticeMenu_Button_2_C not live "
    ...
    auto try_prop = [](std::int32_t& slot, void* cls, const char* name) {
        ...
        if (slot >= 0)
            OPENDOJO_LOG("practice_rename: prop '%s' offset = %d", name, slot);
    };
...
void call_set_raw_text(void* tb, const wchar_t* text) {
    ...
    __try {
        ...
        if (n.fetch_add(1) < 3)
            OPENDOJO_LOG("practice_rename: SEH in SetRawText(tb=0x%p)", tb);
    }
}
...
void set_text_id_shim(void* self, void* frame, void* result) {
    ...
    std::wstring repl;
    {
        ...
        for (const auto& c : g_caps) {
            if (c.tb == self) { repl = c.replacement; break; }
        }
    }
    if (!repl.empty()) {
...
bool install_set_text_id_hook() {
    ...
    *reinterpret_cast<std::uint64_t*>(func_slot) =
    ...
    OPENDOJO_LOG("practice_rename: SetTextID Func patched "
...
void scan_and_apply_rows() {
    ...
    if (!g_r.gryphon_cdo) {
        ...
        if (!g_r.gryphon_cdo) {
            static std::atomic<int> n{0};
            if (n.fetch_add(1) < 4)
                OPENDOJO_LOG("practice_rename: Gryphon CDO not resolved yet");
            return;
        }
    ...
    {
        std::lock_guard<std::mutex> lk(g_caps_mtx);
        for (const auto& c : g_caps) { call_set_raw_text(c.tb, c.replacement.c_str()); ++applied; }
    }
